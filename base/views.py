...
from base.mercado_pago_api import MercadoPagoAPI, FAILURE_STATUS, SUCCESS_STATUS, \
    PENDING_STATUS, IN_PROCESS_STATUS, REJECTED_STATUS
...
def course_list(request, template_name="base/course_list.html"):
    ...
    next_courses = Course.objects.filter(Q(start_date__gte=today) | Q(type='recorded')).exclude(type='admin').order_by('start_date')
    other_courses = Course.objects.exclude(Q(start_date__gte=today) | Q(type='admin')| Q(type='recorded')).order_by('-start_date')
...
def payment_complete(request):
    ...
    if payment_status == FAILURE_STATUS or payment_status == REJECTED_STATUS:
